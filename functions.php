...
/**
 * streamvid functions and definitions
 *
...
if ( ! function_exists( 'jws_setup' ) ) :
	...
	function jws_setup() {
		 ...
		 * If you're building a theme based on streamvid, use a find and replace
		 * to change 'streamvid' to the name of your theme in all the template files.
		 */
		...
		register_nav_menus( array(
			'main_navigation'   => esc_html__( 'Main Menu','streamvid' ),
		) );
...
function jws_widgets_init() {
    ...
    register_sidebar( 
      array(
			'name'          =>  esc_html__( 'Main Sidebar', 'streamvid' ),
			'id'            => 'sidebar-main',
			'description'   =>  esc_html__( 'Add widgets here to appear in your blog.', 'streamvid' ),
    ...
    register_sidebar( 
      array(
			'name'          =>  esc_html__( 'Single Blog Sidebar', 'streamvid' ),
    ...
    register_sidebar( 
      array(
			'name'          =>  esc_html__( 'Shop Page (Left And Right)', 'streamvid' ),
			'id'            => 'sidebar-shop',
			'description'   =>  esc_html__( 'Add widgets here to appear in shop page.', 'streamvid' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
			'before_title'  => '<h5 class="widget-title">',
			'after_title'   => '</h5>',
    ...
    register_sidebar( 
      array(
			'name'          =>  esc_html__( 'Shop Page Filter Modal', 'streamvid' ),
			'id'            => 'sidebar-shop-filter-modal',
			'description'   =>  esc_html__( 'Add widgets here to appear in shop page filter modal.', 'streamvid' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
			'before_title'  => '<h5 class="widget-title">',
			'after_title'   => '</h5>',
    ...
    register_sidebar( 
      array(
			'name'          =>  esc_html__( 'Shop Page Filter Nav Top', 'streamvid' ),
			'id'            => 'sidebar-shop-filter-navtop',
			'description'   =>  esc_html__( 'Add widgets here to appear in shop page filter nav top.', 'streamvid' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
			'before_title'  => '<h5 class="widget-title">',
			'after_title'   => '</h5>',
    ...
    register_sidebar( 
      array(
			'name'          =>  esc_html__( 'Sidebar Remove Filter Active', 'streamvid' ),
...
if(!function_exists('jws_scripts')) {
  function jws_scripts() {
     ...
     wp_enqueue_style( 'select2-min', JWS_URI_PATH . '/assets/css/select2.min.css', array(), $version , 'all' ); 
     ...
     wp_register_style( 'slick', JWS_URI_PATH . '/assets/css/slick.css' );     
     ...
     wp_enqueue_style( 'metropolitano', JWS_URI_PATH . '/assets/font/metropolitano/style.css', array(), $version, 'all' );
   
     wp_enqueue_style( 'font-awesome-pro', JWS_URI_PATH . '/assets/font/awesome/awesome.css' );
     wp_enqueue_style( 'jws-jwsicon', JWS_URI_PATH . '/assets/font/jws_icon/jwsicon.css', array(), $version, 'all' ); 
   
     wp_enqueue_style( 'owl-carousel', JWS_URI_PATH.'/assets/css/owl.carousel.css', array(), $version, 'all' );
     wp_enqueue_script( 'owl-carousel', JWS_URI_PATH. '/assets/js/lib/owl.carousel.js', [], '', true ); 
     wp_enqueue_script( 'toastify', JWS_URI_PATH. '/assets/js/lib/toastify.js', [], '', true ); 
     /** Start Woocommerce **/
     ...
     wp_enqueue_script( 'select2-min', JWS_URI_PATH. '/assets/js/lib/select2.min.js', [] , $version, true );
     ...
     */
     ...
     wp_enqueue_script( 'jws-elementor-video', JWS_URI_PATH. '/assets/js/widget-js/elementor_video.js', [], $version, true );

     /**
     *  Use for all theme
     */
     wp_register_script( 'slick', JWS_URI_PATH. '/assets/js/lib/slick.min.js', [], $version, true );  
    ...
    wp_localize_script(
			...
			array(
                ...
                'add_to_cart_text'  =>  '<span class="text">'.esc_html__('Add to cart','streamvid').'</span>',
                'select_option_text'  =>  '<span class="text">'.esc_html__('Select options','streamvid').'</span>',
                'container_width'   => jws_theme_get_option('container-width'),
                'checkout_url' => function_exists('wc_get_checkout_url') ? wc_get_checkout_url() : '',
                'metera'  =>  esc_html__('Very weak','streamvid'),
                'meterb'  =>  esc_html__('Weak','streamvid'),
                'meterc'  =>  esc_html__('Medium','streamvid'),
                'meterd'  =>  esc_html__('Strong','streamvid'),
...
add_action( 'wp_enqueue_scripts', 'jws_scripts' , 100 );  
} 
