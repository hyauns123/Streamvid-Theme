...
class JWS_PRODUCT_CATEGORY_class extends WC_Widget {
	...
	function __construct() {
        ...
        $this->widget_cssclass    = 'widget-streamvidduct-category';
		$this->widget_description = __( 'It Displays Product Category List', 'streamvid' );
		$this->widget_id          = 'widget-streamvidduct-category';
		$this->widget_name        = __( 'JWS Product Category List', 'streamvid' );
	...
	public function init_settings() {
	   ...
	   $this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'std'   => __( 'Product categories', 'streamvid' ),
				'label' => __( 'Title', 'streamvid' ),
			),
            'hide_empty' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => __( 'Hide empty categories', 'streamvid' ),
			),
			'show_parent_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => __( 'Only show parent category', 'streamvid' ),
			),
            'hidden_category' => array(
				'type'  => 'select',
				'std'     => 'name',
				'label'   => __( 'Hidden category default','streamvid' ),
				'options'   => $product_cat
			),
		);
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="streamvidduct-category-list">
           <div class="container">
               <?php 
                    if(!empty($title)) {
