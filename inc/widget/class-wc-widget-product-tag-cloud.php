...
class JWS_WC_Widget_Product_Tag_Cloud extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'A cloud of your most used product tags.', 'streamvid' );
		$this->widget_id          = 'woocommerce_product_tag_cloud';
		$this->widget_name        = __( 'Product Tag Cloud', 'streamvid' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'std'   => __( 'Product tags', 'streamvid' ),
	...
	public function topic_count_text( $count ) {
		...
		return sprintf( _n( '%s product', '%s products', $count, 'streamvid' ), number_format_i18n( $count ) );
	}
