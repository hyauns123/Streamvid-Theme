...
class CarDealer_Helper_Widget_Product_Search extends WP_Widget {
   ...
   function __construct() {
		$args = array(
			'name'        => esc_html__( 'JWS Product Search', 'streamvid' ),
			'description' => esc_html__( 'Add product search widget in product listing top widget area.', 'streamvid' ),
			'classname'   => 'product_search',
            'customize_selective_refresh' => true,
		);

	   parent::__construct( 'product_search', __( 'JWS Product Search','streamvid' ), $args );
	}
	...
	public function widget( $args, $instance ) {
        ...
        <div class="jws-product-search">
            <form role="search" action="<?php echo esc_url( home_url( '/' ) ); ?>" method="get" class="search-form jws-ajax-search"  data-count="20" data-post_type="product" data-thumbnail="1" data-price="1">
                <input type="text" class="s" placeholder="<?php echo esc_attr_x( 'Search product...', 'placeholder', 'streamvid' ); ?>" value="" name="s" autocomplete="off">
	...
	public function form( $instance ) {
		$title = ! empty( $instance['title'] ) ? $instance['title'] : esc_html__( '', 'streamvid' );
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'streamvid' ); ?></label> 
