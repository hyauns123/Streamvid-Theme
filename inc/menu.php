...
class Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu
{
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $current_object_id = 0)
    {
        ...
        if (!empty($item->_invalid)) {
            ...
            $title = sprintf(esc_html__('%s (Invalid)', 'streamvid'), $item->title);
        } elseif (isset($item->post_status) && 'draft' == $item->post_status) {
            $classes[] = 'pending';
            /* translators: %s: title of menu item in draft status */
            $title = sprintf(esc_html__('%s (Pending)', 'streamvid'), $item->title);
        }
            ...
            <dt class="menu-item-handle">
                ...
                <span class="item-controls">
                    ...
                    <span class="item-order hide-if-js">
                        ...
                        ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'streamvid'); ?>">&#8593;</abbr></a>
                        |
                        <a href="<?php
                        echo wp_nonce_url(
                            add_query_arg(
                        ...
                        ?>" class="item-move-down"><abbr
                                    title="<?php esc_attr_e('Move down', 'streamvid'); ?>">&#8595;</abbr></a>
                    </span>
                    <a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>"
                       title="<?php esc_attr_e('Edit Menu Item', 'streamvid'); ?>" href="<?php
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
            <?php if ('custom' == $item->type) : ?>
                <p class="field-url description description-wide">
                    <label for="edit-menu-item-url-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e('URL', 'streamvid'); ?><br/>
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Navigation Label', 'streamvid'); ?><br/>
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Title Attribute', 'streamvid'); ?><br/>
            ...
            <p class="field-link-target description">
                <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
                    ...
                    <?php esc_html_e('Open link in a new window/tab', 'streamvid'); ?>
                </label>
            </p>
            <p class="field-css-classes description description-thin">
                <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('CSS Classes (optional)', 'streamvid'); ?><br/>
            ...
            <p class="field-xfn description description-thin">
                <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Link Relationship (XFN)', 'streamvid'); ?><br/>
            ...
            <p class="field-css-brand description description-thin">
                <label for="edit-menu-item-brand-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Brand Label Ex( Hot , New ...)', 'streamvid'); ?><br/>
            ...
            <p class="field-brandbg description description-thin">
                <label for="edit-menu-item-brandbg-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Brand Bg Ex(#000000 , red)', 'streamvid'); ?><br/>
            ...
            <?php if (!$depth) { ?>
                <p class="field-description description description-wide">
                    <label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e('Add Shortcode for mega menu', 'streamvid'); ?><br/>
                ...
                <p class="description description-wide jws-design">
                    ...
                    <label for="edit-<?php echo esc_attr($key . '-' . $item_id); ?>">
                        <select id="edit-<?php echo esc_attr($key . '-' . $item_id); ?>"
                            ...
                            <option value="standard" <?php echo ''.($value == 'standard') ? ' selected="selected" ' : ''; ?>><?php esc_html_e('Standard Dropdown', 'streamvid'); ?></option>
                            <option value="mega_menu" <?php echo ''.($value == 'mega_menu') ? ' selected="selected" ' : ''; ?>><?php esc_html_e('Mega With Set Size', 'streamvid'); ?></option>
                            <option value="mega_menu_full_width" <?php echo ''.($value == 'mega_menu_full_width') ? ' selected="selected" ' : ''; ?>><?php esc_html_e('Mega Full Width', 'streamvid'); ?></option>
                        </select>
                    </label>
                </p>
                <?php
            ...
            if (!$depth) {
                ...
                <p class="field-submenu_size description description-thin jws-width">
                    <label for="edit-menu-item-submenu_size-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e('Add Width Mega Menu ', 'streamvid'); ?><br/>
                ...
                <p class="description-thin jws-position">
                    ...
                    <label for="edit-<?php echo esc_attr($key . '-' . $item_id); ?>">
                        <select id="edit-<?php echo esc_attr($key . '-' . $item_id); ?>"
                                class=" <?php echo esc_attr($key); ?>"
                                name="<?php echo esc_attr($key . "[" . $item_id . "]"); ?>">
                            <option value="center" <?php echo ''.($value == 'center') ? ' selected="selected" ' : ''; ?>><?php esc_html_e('Center', 'streamvid'); ?></option>
                            <option value="left" <?php echo ''.($value == 'left') ? ' selected="selected" ' : ''; ?>><?php esc_html_e('Left', 'streamvid'); ?></option>
                            <option value="right" <?php echo ''.($value == 'right') ? ' selected="selected" ' : ''; ?>><?php esc_html_e('Right', 'streamvid'); ?></option>
                        </select>
                    </label>
                </p>
                <?php
            ...
            <div class="menu-item-actions description-wide submitbox">
                <?php if ('custom' != $item->type && $original_title !== false) : ?>
                    <p class="link-to-original">
                        <?php printf(esc_html__('Original: %s', 'streamvid'), '<a href="' . esc_attr($item->url) . '">' . esc_html($original_title) . '</a>'); ?>
                    </p>
                <?php endif; ?>
                ...
                echo wp_nonce_url(
                    ...
                    'delete-menu_item_' . esc_attr($item_id)
                ); ?>"><?php esc_html_e('Remove', 'streamvid'); ?></a> <span class="meta-sep"> | </span> <a
                        ...
                        ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'streamvid'); ?></a>
            </div>
       ...
       if (!empty($brand)) { 
        ...
        $item_output .= $label_out;
         if($depth == 0 && (in_array("menu-item-has-children", $item->classes) || !empty($item->description) )) {
            $item_output .= '<i class="jws-icon-caret-down"></i>';
        }
        $item_output .= '</a>';
        ...
        if ($depth == 0) {
            ...
            if ((!empty($item->description) && !in_array("menu-item-has-children", $item->classes)) && ($sub_type == 'mega_menu' || $sub_type == 'mega_menu_full_width')) {
                $item_output .= "\n$indent<div class=\"sub-menu-dropdown " . $this->type_sub . " " . $this->type_position . "\"><span class='before-menu'></span><div class='jws_mega_sub'>\n";
