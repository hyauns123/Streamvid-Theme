...
class Fonts {
	...
	public static function get_font_groups() {
		if ( null === self::$font_groups ) {
			$font_groups = [
				self::SYSTEM => esc_html__( 'System', 'streamvid' ),
			];

			if ( static::is_google_fonts_enabled() ) {
				$font_groups = array_merge( $font_groups, [
					self::GOOGLE => esc_html__( 'Google', 'streamvid' ),
					self::EARLYACCESS => esc_html__( 'Google (Early Access)', 'streamvid' ),
				] );
			}
