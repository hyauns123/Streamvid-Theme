...
class Jws_Breadcrumbs extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Jws Breadcrumbs', 'streamvid' );
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				[
					...
					'default'	=> '',
					'options'	=> [
						''		=> esc_html__( 'Current page', 'streamvid' ),
						'id'	=> esc_html__( 'Specific page', 'streamvid' ),
					]
				]
			);
            ...
            $this->add_control(
				'show_title',
				[
					'label' 		=> esc_html__( 'Show Title', 'streamvid' ),
			...
			$this->add_control(
				'show_home',
				[
					'label' 		=> esc_html__( 'Show Home', 'streamvid' ),
					'type' 			=> Controls_Manager::SWITCHER,
					'default' 		=> 'yes',
					'label_on' 		=> esc_html__( 'Yes', 'streamvid' ),
					'label_off' 	=> esc_html__( 'No', 'streamvid' ),
			...
			$this->add_control(
				'show_current',
				[
					'label' 		=> esc_html__( 'Show Current', 'streamvid' ),
					'type' 			=> Controls_Manager::SWITCHER,
					'default' 		=> 'yes',
					'label_on' 		=> esc_html__( 'Yes', 'streamvid' ),
					'label_off' 	=> esc_html__( 'No', 'streamvid' ),
			...
			$this->add_control(
				'cpt_crumbs',
				[
					'label' 		=> esc_html__( 'CPT Crumbs', 'streamvid' ),
					'type' 			=> Controls_Manager::SELECT,
					'default' 		=> '',
					'options'		=> [
						'' 			=> esc_html__( 'CPT Name', 'streamvid' ),
						'terms' 	=> esc_html__( 'Taxonomy Terms', 'streamvid' ),
						'both' 		=> esc_html__( 'Both', 'streamvid' ),
					],
				]
			);
			...
			$this->add_control(
				'home_text',
				[
					'label' 		=> esc_html__( 'Home Text', 'streamvid' ),
					'type' 			=> Controls_Manager::TEXT,
					'default' 		=> esc_html__( 'Homepage', 'streamvid' ),
					'dynamic'		=> [
						'active'	=> true,
		...
		$this->start_controls_section(
			'section_separator',
			[
				'label' => esc_html__( 'Separator', 'streamvid' ),
			]
		);
			...
			$this->add_control(
				...
				[
					...
					'default' 	=> 'icon',
					'options' 	=> [
						'text' 		=> esc_html__( 'Text', 'streamvid' ),
						'icon' 		=> esc_html__( 'Icon', 'streamvid' ),
					],
				]
			);
		...
		$this->start_controls_section(
			'section_item_style',
			[
				'label' 	=> esc_html__( 'Crumbs', 'streamvid' ),
			...
			$this->add_responsive_control(
				'items_text_align',
				[
					'label' 		=> esc_html__( 'Align Text', 'streamvid' ),
			...
			$this->start_controls_tab( 'crumb_default', [ 'label' => esc_html__( 'Default', 'streamvid' ) ] );
			...
			$this->start_controls_tab( 'crumb_hover', [ 'label' => esc_html__( 'Hover', 'streamvid' ) ] );
		...
		$this->start_controls_section(
			...
			[
				'label' 	=> esc_html__( 'Separators', 'streamvid' ),
				'tab' 		=> Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->start_controls_section(
			...
			[
				'label' 	=> esc_html__( 'Current', 'streamvid' ),
				'tab' 		=> Controls_Manager::TAB_STYLE,
			]
		);
	...
	protected function render() {
		...
		if ( $_query ) {
			if ( $_query->have_posts() ) {
				...
				_e( 'Post or page not found', 'streamvid' );
		...
		if ( ! $query->is_front_page() ) { ?>
            ...
            if ( is_search()) {
				...
				$this->render_item( 'search', [
					...
					'separator'	=> false,
					'key' 		=> 'search',
					'content' 	=> sprintf( esc_html__( 'Search results for: %s', 'streamvid' ), get_search_query() ),
				] );
			
			} 
			 else if ( $query->is_archive() && ! $query->is_tax() && ! $query->is_category() && ! $query->is_tag() && ! $query->is_date() && ! $query->is_author() ) {
				...
				$this->render_item( 'tag', [
					...
					'content' 	=> sprintf( esc_html__( 'Tag: %s', 'streamvid' ), $get_term_name ),
				] );
				...
				$this->render_item( 'year', [
					...
					'key' 		=> 'year',
					'ids' 		=> [ get_the_time('Y') ],
					'content' 	=> sprintf( esc_html__( '%s Archives', 'streamvid' ), get_the_time('Y') ),
				...
				$this->render_item( 'month', [
					...
					'key' 		=> 'month',
					'ids' 		=> [ get_the_time('m') ],
					'content' 	=> sprintf( esc_html__( '%s Archives', 'streamvid' ), get_the_time('F') ),
				...
				$this->render_item( 'day', [
					...
					'key' 		=> 'day',
					'ids' 		=> [ get_the_time('j') ],
					'content' 	=> sprintf( esc_html__( '%1$s %2$s Archives', 'streamvid' ), get_the_time('F'), get_the_time('jS') ),
				] );
				...
				$this->render_item( 'author', [
					...
					'content' 	=> sprintf( esc_html__( 'Author: %s', 'streamvid' ), $userdata->display_name ),
				] );
				...
				$this->render_item( '404', [
					...
					'separator'	=> false,
					'key' 		=> '404',
					'content' 	=> esc_html__( 'Page not found', 'streamvid' ),
				] );
			} elseif ( is_home() ) { 
