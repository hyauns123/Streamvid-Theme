...
if($settings['layout'] == 'carousel') {
    ...
    $settings['autoplay'] ? $settings['autoplay'] : $settings['autoplay'] = 'false';
        ...
        "nav": '.$settings['show_nav'].', 
        "dots":'.$settings['show_pag'].', 
        "center":'.$center.', 
        "loop":'.$infinite.',
        "smartSpeed": '.$settings['speed'].', 
        "responsive":{
            "1024":{"items": '.$settings['slides_to_show'].',"slideBy": '.$settings['scroll'].'},
            "768":{"items": '.$settings['slides_to_show_tablet'].',"slideBy": '.$settings['scroll'].'},
            "0":{"items": '.$settings['slides_to_show_mobile'].',"slideBy": '.$settings['scroll'].'}
        }}\''; 
        ...
        <div class="jws-head-filter has-tabs <?php echo esc_attr($settings['title'] ? 'has-border' : '');?>">
            <ul class="jws-ajax-load filter-asset ct_ul_ol">
              ...
              switch ( $settings['default_asset'] ) {
                 case 'featured':
                 $asset_title = esc_html__( 'Featured', 'streamvid' );
                 break;
                 case 'onsale':
                 $asset_title = esc_html__( 'On Sale', 'streamvid' );
                 break;
                 case 'deal':
                 $asset_title = esc_html__( 'Deal', 'streamvid' );
                 break;
                 case 'latest':
                 $asset_title = esc_html__( 'New Arrivals', 'streamvid' );
                 break;
                 case 'best-selling':
                 $asset_title = esc_html__( 'Best sellers', 'streamvid' );
                 break;
                 case 'toprate':
                 $asset_title = esc_html__( 'Top Rate', 'streamvid' );
                 break;
                 default:
                 break;
             }
            ...
            if ( $settings['filter_assets'] ) {
             ...
             foreach ( $settings['filter_assets'] as $val ) {
                switch ( $val ) {
                   case 'featured':
                   $html .= $settings['default_asset'] != 'featured' ? '<li><a href="#" class=" ajax-load" data-type="asset_type" data-value="featured" title="' . esc_html__( 'Featured', 'streamvid' ) . '">' . esc_html__( 'Featured', 'streamvid' ) . '</a></li>' : '';
                   break;
                   case 'onsale':
                   $html .= $settings['default_asset'] != 'onsale' ? '<li><a href="#" class=" ajax-load" data-type="asset_type" data-value="onsale" title="' . esc_html__( 'On Sale', 'streamvid' ) . '">' . esc_html__( 'On Sale', 'streamvid' ) . '</a></li>' : '';
                   break;
                   case 'deal':
                   $html .= $settings['default_asset'] != 'deal' ? '<li><a href="#" class=" ajax-load" data-type="asset_type" data-value="deal" title="' . esc_html__( 'Deal', 'streamvid' ) . '">' . esc_html__( 'Deal', 'streamvid' ) . '</a></li>' : '';
                   break;
                   case 'latest':
                   $html .= $settings['default_asset'] != 'latest' ? '<li><a href="#" class=" ajax-load" data-type="asset_type" data-value="latest" title="' . esc_html__( 'New Arrivals', 'streamvid' ) . '">' . esc_html__( 'New Arrivals', 'streamvid' ) . '</a></li>' : '';
                   break;
                   case 'best-selling':
                   $html .= $settings['default_asset'] != 'best-selling' ? '<li><a href="#" class=" ajax-load" data-type="asset_type" data-value="best-selling" title="' . esc_html__( 'Best sellers', 'streamvid' ) . '">' . esc_html__( 'Best Seller', 'streamvid' ) . '</a></li>' : '';
                   break;
                   case 'toprate':
                   $html .= $settings['default_asset'] != 'toprate' ? '<li><a href="#" class=" ajax-load" data-type="asset_type" data-value="toprate" title="' . esc_html__( 'Top Rate', 'streamvid' ) . '">' . esc_html__( 'Top Rate', 'streamvid' ) . '</a></li>' : '';
                   break;
                   default:
                   break;
               }
...
<div class="products-tab <?php echo esc_attr($class) ?>">
    ...
    <?php
        if($settings['layout'] == 'carousel') echo '<div class="carousel owl-carousel" '.$data_slick.'>';  
...
<?php if ( $product_query->max_num_pages > 1 && $settings['pagination'] == 'ajaxload') { ?>
    <div class="jws_product_pagination">
         <a class="jws-products-load-more" href="#">
            <span class="more_product"><?php echo esc_html__('Load More','streamvid'); ?></span>
         </a>
    </div>
<?php } ?>
