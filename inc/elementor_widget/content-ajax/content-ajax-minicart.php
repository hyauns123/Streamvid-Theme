...
<div class="cart-head">
    <label><?php echo esc_html__('Your Cart','streamvid'); ?><span class="count"><?php echo esc_attr($count); ?></span></label>
...
<ul class="cart_list">
                    ...
                    <li class="jws-cart-panel-item-<?php echo esc_attr( $cart_item_key ); ?>" class="woocommerce-mini-cart-item <?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key ) ); ?>">
                        ...
                        <div class="jws-cart-panel-item-content">
                            ...
                            <?php
                              echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf(
                                        ...
                                        esc_url( wc_get_cart_remove_url( $cart_item_key ) ),
                                         esc_html__( 'Remove this item', 'streamvid' ),
                                        esc_attr( $product_id ),
                            ...
                            <?php if ( $_product->is_sold_individually() ) : ?>
                                    <?php
                                        echo apply_filters( 'woocommerce_widget_cart_item_quantity', '<span class="quantity">' . esc_html__( 'Qty', 'streamvid' ) . ': ' . $cart_item['quantity'] . '</span>', $cart_item, $cart_item_key );
                                    ?>
                            <?php else: ?>
    ...
    <?php if (WC()->cart->is_empty()) : ?>
         <li class="cart_empty">
            ...
            <?php esc_html_e( 'No Products in the Cart.', 'streamvid' ); ?>
            <a class="button-default" href="<?php echo get_permalink( wc_get_page_id( 'shop' ) ); ?>"><span><?php echo esc_html__('Continue Shopping','streamvid'); ?></span></a>
         </li>
    <?php endif; ?>
...
<div class="jws-cart-panel-summary">
    ...
    <div class="jws-cart-panel-summary-inner">
        ...
        <p class="woocommerce-mini-cart__total total">
            <strong><?php esc_html_e( 'Subtotal', 'streamvid' ); ?>:</strong>
        ...
        <a href="<?php echo esc_url( wc_get_cart_url() ); ?>" class="button button-custom edit-cart"><?php esc_html_e( 'View cart', 'streamvid' ); ?></a>
        </p>
        <p class="woocommerce-mini-cart__buttons buttons_checkout in_product">
            <a href="<?php echo esc_url( wc_get_checkout_url() ); ?>" class="button checkout button-default wc-forward"><?php esc_html_e( 'Checkout', 'streamvid' ); ?></a>
        </p>
...
function bbloomer_free_shipping_cart_notice_zones()
{
    ...
    if (isset($min_amounts) && is_array($min_amounts)) {
        ...
        if ($current < $min_amount) { 
            ...
            $html .= '<div class="jws_shipping_wap">' .esc_html__('Spend ', 'streamvid') .  wc_price($min_amount - $current) . sprintf('%s %s',  __(' to reach', 'streamvid') , '<span class="text-free">'.__(' free shipping', 'streamvid').'</span>' ).  '</span></div>';
        } else {
            $html .= '<div class="jws_shipping_wap"><span class="fa fa-check-square"></span>' . esc_html__('Free standard delivery', 'streamvid') . '</div>';
        }
        $html .= '</div>';
        return $html;
    }
