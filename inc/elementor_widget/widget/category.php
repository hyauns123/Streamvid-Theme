...
class WP_Widget_Categories_Custom extends WP_Widget {
  ...
  function __construct() {
    $widget_ops = array( 'classname' => 'widget_categories widget_categories_custom', 'description' => __( "A list of categories, with slightly tweaked output.", 'streamvid'  ) );
    parent::__construct( 'categories_custom', __( 'Categories Custom', 'streamvid' ), $widget_ops );
  }
  ...
  function form( $instance ) {
    ...
    $dropdown = false;
?>
    <p><label for="<?php echo wp_kses_post($this->get_field_id('title', 'streamvid' )); ?>"><?php _e( 'Title:', 'streamvid'  ); ?></label>
    ...
    <label for="<?php echo wp_kses_post($this->get_field_id('count')); ?>"><?php _e( 'Show post counts', 'streamvid'  ); ?></label><br />
<?php
  }
