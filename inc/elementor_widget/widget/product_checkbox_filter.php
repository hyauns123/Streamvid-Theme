...
class jws_CHECKBOX_FILTER_class extends WP_Widget {
	...
	function __construct() {
		$args = array(
			'name'        => esc_html__( 'Jws Filter Product By Taxonomy', 'streamvid' ),
			'description' => esc_html__( 'It displays Filter', 'streamvid' ),
	...
	function form( $instance ) {
		$instance = wp_parse_args(
			(array) $instance,
			array(
				'title'       => esc_html__( 'Type', 'streamvid' ), // Legacy.
		...
		<p>
			...
			<input class="widefat" id="<?php echo esc_attr( esc_attr( $this->get_field_id( 'title' ) ) ); ?>"
		      ...
		      <label for="<?php echo esc_attr($this->get_field_id( 'type' )); ?> "><?php esc_html_e('Type Filter:','streamvid'); ?></label>
            <select id="<?php echo esc_attr($this->get_field_id( 'type' )) ?>" name="<?php echo esc_attr($this->get_field_name( 'type' )); ?>">
                 <option <?php selected( $type, 'category' ); ?> value="category" ><?php echo esc_attr('category','streamvid'); ?></option>
                 <option <?php selected( $type, 'brand' ); ?> value="brand" ><?php echo esc_attr('brand','streamvid'); ?></option>
                 <option <?php selected( $type, 'features' ); ?> value="features" ><?php echo esc_attr('features','streamvid'); ?></option>
            </select>
		</p>
		<?php
	}
	...
	function widget( $args, $instance ) {
		...
		$instance = wp_parse_args(
			(array) $instance,
			array(
				'title'       => esc_html__( 'PRICE', 'streamvid' ), // Legacy.
		...
		<div class="type checkbox">
            ...
            if($type == 'brand') {
                $tax = 'product_brand';
                $all = esc_attr__('All brand','streamvid');
            }elseif($type == 'category') {
                $tax = 'product_cat';
                $all = esc_attr__('All Category','streamvid');
            }elseif($type == 'features') {
                $tax = 'product_features';
                $all = esc_attr__('All Features','streamvid');
            }
